...
using Mf.Intr.Application.Services.SboServiceLayer.Exceptions;
...
/// </remarks>
public class SboServiceLayerConnection : ISboServiceLayerConnection
{
    #region Fields
    private DateTime _lastRequest = DateTime.MinValue;
    private ISboServiceLayerLoginResponse _loginResponse = new SboServiceLayerLoginResponse();
    ...
    /// </summary>
    public ISboServiceLayerLoginResponse? LoginResponse
    {
        // Returns a new object so the login control can't be manipulated externally
        get => string.IsNullOrEmpty(_loginResponse.SessionId) ? null : new SboServiceLayerLoginResponse(
            _loginResponse.SessionId, 
            _loginResponse.Version, 
            _loginResponse.SessionTimeout, 
            _loginResponse.LastLogin);
    ...
    #region Constructors
    /// <summary>
    /// Initializes a new instance of the <see cref="SboServiceLayerConnection"/> class.
    /// Only one instance per company/user should be used in the application.
    /// </summary>
    /// <param name="flurlClient">
    /// Flurl http client containing definition and service Layer root URI.
    /// </param>
    /// <param name="slConEntity">
    /// Service layer parameters
    /// </param>
    public SboServiceLayerConnection(IFlurlClient flurlClient, ILogger logger, SboServiceLayerConnectionEntity slConEntity)
    {
        _flurlClient = flurlClient;

        //The Service Layer root URI.The expected format is https://[server]:[port]/b1s/[version]
        ServiceLayerRoot = new Uri(slConEntity.Address);

        // The Company database (schema) to connect to.
        CompanyDB = slConEntity.Company;

        // The SAP user to be used for the Service Layer authentication.
        UserName = slConEntity.User;

        // The password for the provided user.
        Password = slConEntity.Password;
        ...
        NumberOfAttempts = slConEntity.NumberOfAttempts ?? 3;
    }
    ...
    ///// If the response code is 401 (Unauthorized), a login request will be performed before the new attempt.
    ///// </param>
    //public SboServiceLayerConnection(Uri serviceLayerRoot, Func<string, string> getServiceLayerConnectionContext, int sessionTimeout = 30, int numberOfAttempts = 3)
    //{
    //    ServiceLayerRoot = serviceLayerRoot;
    //    NumberOfAttempts = numberOfAttempts;
    //    LoginResponse = new SboServiceLayerLoginResponse();
    ...
    ///// <summary>
    ///// Initializes a new instance of the <see cref="SboServiceLayerConnection"/> class using Single Sign-On (SSO) authentication.
    ///// </summary>
    ///// <param name="serviceLayerRoot">
    ///// The Service Layer root URI. The expected format is https://[server]:[port]/b1s/[version]
    ///// </param>
    ...
    ///// If the response code is 401 (Unauthorized), a login request will be performed before the new attempt.
    ///// </param>
    //public SboServiceLayerConnection(string serviceLayerRoot, Func<string, string> getServiceLayerConnectionContext, int sessionTimeout = 30, int numberOfAttempts = 3)
    ...
    /// </param>
    public async Task<ISboServiceLayerLoginResponse?> LoginAsync(bool forceLogin = false) => await ExecuteLoginAsync(forceLogin, true);
    ...
    /// </param>
    private async Task<ISboServiceLayerLoginResponse?> ExecuteLoginAsync(bool forceLogin = false, bool expectReturn = false)
    {
        ...
        try
        {
            ...
            {
                var loginResponse = await _flurlClient.Request("Login")
                    .WithCookies(out var cookieJar)
                    .PostJsonAsync(new { CompanyDB, UserName, Password, Language })
                    .ReceiveJson<SboServiceLayerLoginResponse>();
            ...
            else
            {
                ...
                string? connectionContext = _getServiceLayerConnectionContext?.Invoke(ServiceLayerRoot.ToString());
                if (connectionContext == null)
                {
                    throw new IntegrationException("ConnectionContext is null when trying to get using SSO");
                }
        ...
        catch (FlurlHttpException ex)
        {
            try
            {
                ...
                var response = await ex.GetResponseJsonAsync<SboServiceLayerErrorResponse>();
                throw new SboServiceLayerException(response.Error.Message.Value, response.Error, ex);
            }
            catch { throw ex; }
        }
        finally
    ...
    private CookieJar CreateCookieJarFromConnectionContext(string connectionContext)
    {
        ...
        foreach (var cookie in cookies)
        {
            ...
            {
                cookieJar.AddOrReplace(new FlurlCookie(cookieKeyValue[0], cookieKeyValue[1], _flurlClient.Request("Login").Url.ToString()) { HttpOnly = true, Secure = true });
            }
            else if (cookieKeyValue[0].Equals("ROUTEID", StringComparison.OrdinalIgnoreCase))
            {
                cookieJar.AddOrReplace(new FlurlCookie(cookieKeyValue[0], cookieKeyValue[1], _flurlClient.Request("Login").Url.ToString()) { Path = "/", Secure = true });
            }
        }
    ...
    public async Task LogoutAsync()
    {
        ...
        try
        {
            await _flurlClient.Request("Logout").WithCookies(Cookies).PostAsync();
            _loginResponse = new SboServiceLayerLoginResponse();
            _lastRequest = default;
            Cookies = null;
        }
        catch (FlurlHttpException ex)
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="SboServiceLayerRequest"/> class that represents a request to the associated <see cref="SboServiceLayerConnection"/>. 
    /// </summary>
    /// <param name="resource">
    /// The resource name to be requested.
    /// </param>
    public ISboServiceLayerRequest Request(string resource) =>
        new SboServiceLayerRequest(this, _flurlClient.Request(resource));
    ...
    /// </param>
    public ISboServiceLayerRequest Request(string resource, object id) =>
        new SboServiceLayerRequest(this, _flurlClient.Request(id is string ? $"{resource}('{id}')" : $"{resource}({id})"));
        ...
        for (int i = 0; i < NumberOfAttempts || loginReattempted; i++)
        {
            ...
            catch (FlurlHttpException ex)
            {
                ...
                try
                {
                    ...
                    var response = await ex.GetResponseJsonAsync<SboServiceLayerErrorResponse>();
                    exceptions.Add(new SboServiceLayerException(response.Error.Message.Value, response.Error, ex));
                }
                catch
                {
                    exceptions.Add(ex);
    ...
    /// </returns>
    public async Task<ISboServiceLayerPingResponse> PingAsync() => await ExecutePingAsync("ping/");
    ...
    /// A <see cref="ISboServiceLayerPingResponse"/> object containing the response details.
    /// </returns>
    public async Task<ISboServiceLayerPingResponse> PingNodeAsync(int? node = null) =>
    ...
    /// </summary>
    private async Task<ISboServiceLayerPingResponse> ExecutePingAsync(string path)
    {
        try
        {
            var response = await _flurlClient.Request(path).GetAsync();

            var pingResponse = await response.GetJsonAsync<SboServiceLayerPingResponse>();
        ...
        catch (FlurlHttpException ex)
        {
            ...
            {
                if (ex.Call.HttpResponseMessage == null) throw;
                var pingResponse = await ex.GetResponseJsonAsync<SboServiceLayerPingResponse>();
    ...
    /// </param>
    /// <returns>
    /// A <see cref="SboServiceLayerAttachment"/> object with information about the created attachment entry.
    /// </returns>
    public async Task<ISboServiceLayerAttachment> PostAttachmentAsync(string path) =>
    ...
    /// A <see cref="SboServiceLayerAttachment"/> object with information about the created attachment entry.
    /// </returns>
    public async Task<ISboServiceLayerAttachment> PostAttachmentAsync(string fileName, byte[] file) =>
    ...
    /// A <see cref="SboServiceLayerAttachment"/> object with information about the created attachment entry.
    /// </returns>
    public async Task<ISboServiceLayerAttachment> PostAttachmentAsync(string fileName, Stream file) =>
    ...
    /// A <see cref="SboServiceLayerAttachment"/> object with information about the created attachment entry.
    /// </returns>
    public async Task<ISboServiceLayerAttachment> PostAttachmentsAsync(IDictionary<string, byte[]> files) =>
    ...
    /// A <see cref="SboServiceLayerAttachment"/> object with information about the created attachment entry.
    /// </returns>
    public async Task<ISboServiceLayerAttachment> PostAttachmentsAsync(IDictionary<string, Stream> files)
    {
        return await ExecuteRequest(async () =>
        {
            ...
            var result = await _flurlClient.Request("Attachments2")
                .WithCookies(Cookies)
                ...
                .ReceiveJson<SboServiceLayerAttachment>();
    ...
    public async Task PatchAttachmentsAsync(int attachmentEntry, IDictionary<string, Stream> files)
    {
        await ExecuteRequest(async () =>
        {
            ...
            var result = await _flurlClient.Request($"Attachments2({attachmentEntry})")
                .WithCookies(Cookies)
    ...
    public async Task<byte[]> GetAttachmentAsBytesAsync(int attachmentEntry, string? fileName = null)
    {
        return await ExecuteRequest(async () =>
        {
            var file = await _flurlClient.Request($"Attachments2({attachmentEntry})/$value")
    ...
    public ISboServiceLayerBatchRequest CreateBatchRequest(HttpMethod httpMethod, string resource, object? data = null, int? contentID = null)
    {
        return new SboServiceLayerBatchRequest(httpMethod, resource, data, contentID);
    }
    ...
    /// </remarks>
    /// <param name="requests">
    /// <see cref="SboServiceLayerBatchRequest"/> instances to be sent in the batch.
    /// </param>
    /// <returns>
    /// An <see cref="HttpResponseMessage"/> array containg the response messages of the batch request. 
    /// </returns>
    public async Task<HttpResponseMessage[]> PostBatchAsync(params ISboServiceLayerBatchRequest[] requests)
    {
    ...
    /// An <see cref="HttpResponseMessage"/> array containg the response messages of the batch request. 
    /// </returns>
    public async Task<HttpResponseMessage[]> PostBatchAsync(IEnumerable<ISboServiceLayerBatchRequest> requests, bool singleChangeSet = true)
    {
        ...
        return await ExecuteRequest(async () =>
        {
            ...
            var response = singleChangeSet
                ? await _flurlClient.Request("$batch")
                    .WithCookies(Cookies)
    ...
    /// </summary>
    private MultipartContent BuildMixedMultipartContent(IEnumerable<ISboServiceLayerBatchRequest> requests)
    {
        string boundary = "changeset_" + Guid.NewGuid();
    ...
    /// </summary>
    private MultipartContent BuildMixedMultipartContent(ISboServiceLayerBatchRequest batchRequest, string boundary)
    {
        var multipartContent = new MultipartContent("mixed", boundary);
        BuildRequestForMultipartContent(multipartContent, batchRequest);
        return multipartContent;
    ...
    /// <summary>
    /// Builds the <see cref="HttpRequestMessage"/> from a given <see cref="SboServiceLayerBatchRequest"/> and adds it to the <see cref="MultipartContent"/> instance.
    /// </summary>
    private void BuildRequestForMultipartContent(MultipartContent multipartContent, ISboServiceLayerBatchRequest batchRequest)
    {
        var sboBatchRequest = (SboServiceLayerBatchRequest)batchRequest;
        var request = new HttpRequestMessage(sboBatchRequest.HttpMethod, Url.Combine(ServiceLayerRoot.ToString(), sboBatchRequest.Resource));

        if (sboBatchRequest.HttpVersion != null)
            request.Version = sboBatchRequest.HttpVersion;

        foreach (var header in sboBatchRequest.Headers)
            request.Headers.Add(header.Key, header.Value);

        if (sboBatchRequest.Data != null)
            request.Content = sboBatchRequest.Data is string dataString
                ? new StringContent(dataString, sboBatchRequest.Encoding, "application/json")
                : new StringContent(JsonSerializer.Serialize(sboBatchRequest.Data, sboBatchRequest.JsonSerializerOptions), sboBatchRequest.Encoding, "application/json");

        var innerContent = new HttpMessageContent(request);
        innerContent.Headers.Add("content-transfer-encoding", "binary");

        if (sboBatchRequest.ContentID.HasValue)
            innerContent.Headers.Add("Content-ID", sboBatchRequest.ContentID.ToString());
